...
// Handle return action (when ?return=Loan_ID is present)
if (isset($_GET['return'])) {
    $loan_id = intval($_GET['return']);
    $msg = 'returned';

    // Verify the loan belongs to this student and is currently active
    $verify = $conn->prepare("
        ...
        LIMIT 1
    ");
    $verify->bind_param("ii", $loan_id, $user_id);
    $verify->execute();
    $loan = $verify->get_result()->fetch_assoc();

    if ($loan) {
        // Mark loan as returned and stamp Date_Returned
...
// Handle reserve action (when ?reserve=Book_ID is present)
if (isset($_GET['reserve'])) {
    $book_id = intval($_GET['reserve']);
    $msg = 'reserved';

    // Prevent duplicate reservations and borrowing conflicts
    // 1) Check if the user already has an active reservation for this book
    ...
    // 2) Check if the user currently holds an active loan for this book
...
");
$loansStmt->bind_param("i", $user_id);
$loansStmt->execute();
$loans = $loansStmt->get_result();

// Fetch books eligible for reservation: not Available (e.g., currently borrowed, repair, etc.)
...
$toastMap = [
    ...
    'reserved' => ['Reservation created. Awaiting approval.', 'info'],
    'already'  => ['You already have this book (loan or reservation).', 'error'],
    'error'    => ['Something went wrong. Please try again.', 'error'],
];
$msgKey = isset($_GET['msg']) ? $_GET['msg'] : null;
$toast = $msgKey && isset($toastMap[$msgKey]) ? $toastMap[$msgKey] : null;
?>
<style>
